...
class Drone_state():
	...
	def cb_drone_state(self,pose):
		...
		nv = (self.prev_z - n)/0.1
		self.set_drone_state([n, nv])   #set z value obtained in bracket	
		
			

class DQNCartPoleSolver():
	def __init__(self, n_episodes=1000, n_win_ticks=195, max_env_steps=None, gamma=0.8, epsilon=1.0, epsilon_min=0.01, epsilon_log_decay=0.995, alpha=0.01, alpha_decay=0.9, batch_size=64, monitor=False, quiet=False):
	...
	def get_epsilon(self, t):
		...
		if(t<200):
			...
			ep = self.epsilon_min + (self.epsilon_max - self.epsilon_min)*(math.exp(-0.01*(t-200)))
		return ep		
...
class validate():
	...
	def run(self):
		...
		model = load_weights("drone_ddqn.h5")
